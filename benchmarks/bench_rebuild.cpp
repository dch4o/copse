...
// Microbenchmark: rebuild tail latency at N=250k. Pairs partial-rebuild
...
TEST_CASE("Bench: partial rebuild tail latency, 250k points", "[!benchmark][rebuild][partial]") {
    // Steady-state proxy: a capacity-250k tree pre-filled to full, then a
    ...
    BENCHMARK_ADVANCED("partial rebuild (3, N=250k)")
...
TEST_CASE("Bench: rebuild_all reference, 250k points", "[!benchmark][rebuild][full]") {
    // Reference upper bound: explicit full rebuild over 250k live points.
    // Useful as a cap for "how bad can the worst rebuild be" when comparing
    // against the partial-rebuild row above.

    BENCHMARK_ADVANCED("rebuild_all (3, N=250k)")
    (Catch::Benchmark::Chronometer meter) {
        const auto fill = make_points(kCapacity, kSeed ^ 0x1u, kCoordExtent);
        Tree       tree{make_config(kCapacity)};
        tree.insert(fill);
...
TEST_CASE("Bench: rebuild N sweep (partial + full)", "[!benchmark][rebuild][sweep]") {
    ...
    // (same shape as the 250k case above) and rebuild_all reference. Each
    ...
    BENCHMARK_ADVANCED("partial rebuild (3, N=250k)")
    (Catch::Benchmark::Chronometer meter) {
        const auto fill  = make_points(250'000, kSeed ^ 0x18u, kCoordExtent);
...
TEST_CASE("Bench: degenerate cluster insert (LeafBucket overflow path), 250k points",
    ...
    BENCHMARK_ADVANCED("degenerate cluster insert (3, N=250k, 1k batch)")
    (Catch::Benchmark::Chronometer meter) {
...
TEST_CASE("Bench: tombstone-triggered partial rebuild, 250k points", "[!benchmark][rebuild][tombstone]") {
    // Best-effort tombstone-fraction trigger. Pre-fill 250k, remove ~30% of the
    ...
    constexpr std::size_t kRemoveFraction = 75'000;  // ~30% of 250k
    constexpr std::size_t kSmallBatch     = 1'000;

    BENCHMARK_ADVANCED("tombstone-triggered partial rebuild (3, N=250k, best-effort)")
    (Catch::Benchmark::Chronometer meter) {
