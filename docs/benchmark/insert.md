...
> Run date: 2026-06-16 · Source: `benchmarks/bench_insert.cpp`
...
| -------- | -------------------- | ----------: | -------: | ---------------: |
| 50k      | cold (no eviction)   |     2.41 ms |  42.5 µs |           241 ns |
| 50k      | warm (FIFO eviction) |     2.36 ms |  42.0 µs |           236 ns |
| 100k     | cold (no eviction)   |     2.51 ms |  36.0 µs |           251 ns |
| 100k     | warm (FIFO eviction) |     3.04 ms |   119 µs |           304 ns |
| 500k     | cold (no eviction)   |     2.88 ms |   273 µs |           288 ns |
| 500k     | warm (FIFO eviction) |     8.27 ms |   213 µs |           827 ns |
| 1M       | cold (no eviction)   |     3.92 ms |   324 µs |           392 ns |
| 1M       | warm (FIFO eviction) |    11.48 ms |   322 µs |          1.15 µs |
...
xychart-beta
    ...
    y-axis "time (ms)" 0 --> 12
    bar [2.41, 2.51, 2.88, 3.92]
    line [2.36, 3.04, 8.27, 11.48]
```
...
| -------- | ---------- | -----------: | --------: | ---------------: |
|     100k |        100 |      52.3 µs |   4.52 µs |           523 ns |
|     100k |      1,000 |       284 µs |   20.0 µs |           284 ns |
|     100k |     10,000 |     2.801 ms |   32.9 µs |           280 ns |
|       1M |        100 |     10.28 ms |   81.3 µs |           103 µs |
|       1M |      1,000 |     10.41 ms |    532 µs |          10.4 µs |
|       1M |     10,000 |     13.31 ms |    397 µs |          1.33 µs |

### Warm (FIFO eviction), small batch into full tree

| Capacity | Prefill | Batch size |  Mean / call |    Stddev | Per-point (mean) |
| -------- | ------- | ---------- | -----------: | --------: | ---------------: |
|       1M |      1M |      1,000 |     3.710 ms |   97.2 µs |          3.71 µs |

## What this tells us

**Per-tree construction dominates cold time at large capacity.** At
`capacity = 1M` the cold per-call time is ~10–13 ms regardless of
batch size in the 100–10k range; the construction step pays ~10 ms
on its own. At `capacity = 100k` construction shrinks below the batch
cost, and per-point time drops from 523 ns (batch 100) to 280 ns
...
2.41 ms); the warm-vs-cold gap at large `N` comes from the end-of-batch
recursive sweep, not from eviction per se.

**Warm scaling** (batch = 10k):
- 50k → 100k: 2.36 → 3.04 ms (~1.3×) — well sub-`log N`
- 100k → 500k: 3.04 → 8.27 ms (~2.7×)
- 500k → 1M: 8.27 → 11.48 ms (~1.4×)
