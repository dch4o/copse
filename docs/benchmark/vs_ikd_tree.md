# tkd-tree vs. ikd-Tree benchmark

> Run date: 2026-06-16 · Source: `benchmarks/bench_perf_ikd.cpp`

Comparative microbenchmark of `topiary::KDTree3` against the HKU MARS
...
- **Run modes:**
  - **(a) tkd** — `topiary::KDTree3`, single-thread, inline partial rebuild.
...
Bar = tkd. Lines = ikd bg-off / bg-on. Tied at 10k; tkd ~1.54× faster at 100k.
...
Bar = tkd. Lines = ikd bg-off / bg-on. tkd ~1.8–1.9× faster.
...
Bar = tkd. Lines = ikd bg-off / bg-on. tkd ~1.5× (10k) → ~2.1× (100k) faster:
...
Bar = tkd. Lines = ikd bg-off / bg-on. **tkd now beats ikd's single-thread mode
...
Bar = tkd. Lines = ikd bg-off / bg-on. **bg-on's wall < CPU here (12.37 vs 9.49
...
so tkd (single-thread) is the fastest in wall at 100k (194 ms) — its partial
...
10 frames; tkd capacity holds the base map plus all inserts (no FIFO eviction).

| N    | Mode           | Wall / frame | CPU / frame | Wall stddev |
| ---- | -------------- | -----------: | ----------: | ----------: |
| 10k  | (a) tkd        |     0.500 ms |    0.500 ms |     8.6 µs  |
| 10k  | (b) ikd bg-off |     0.826 ms |    0.829 ms |    21.4 µs  |
| 10k  | (c) ikd bg-on  |     0.821 ms |    0.831 ms |     7.3 µs  |
| 100k | (a) tkd        |     0.820 ms |    0.820 ms |    12.3 µs  |
...
Bar = tkd. Lines = ikd bg-off / bg-on. tkd ~1.6× (10k) → ~1.3× (100k) faster.
...
| ---- | -------------- | --------: | -------------: | ----------: |
| 10k  | (a) tkd        |  ~0.1 MB  |        0.47 MB |        49.0 |
| 10k  | (b)/(c) ikd    |  41.96 MB |        1.30 MB |       136.0 |
| 100k | (a) tkd        |  ~0–4 MB  |        4.67 MB |        49.0 |
| 100k | (b)/(c) ikd    |  41.96 MB |       12.97 MB |       136.0 |

tkd is **~2.8× leaner per point** (49 vs 136 bytes) and carries no fixed
...
of ~136 MB of ikd nodes vs ~49 MB for tkd. The gap is structural: tkd stores
points in a flat SoA (≈17 B/slot + a small bucketed node array), ikd chases
136 B pointer-laden nodes. (tkd's RSS delta is within page-noise at these N; the
...
- **Box convention** — both use half-open `min <= p < max`. **tkd FIFO eviction**
  has no ikd analog; the mixed and bulk workloads size tkd capacity above the
  live count so eviction never fires.
- Results are sets, not sequences; tie-break order is not compared.

## What this tells us

- **tkd wins every read/insert path, the mixed SLAM loop, and memory.** Insert
...
- **Batched spatial delete now favours tkd over single-thread ikd** (~2.3× at
  100k). Scoping the rebuild trigger to the touched nodes, then batching the 64
  deletes into one end-of-batch rebuild, took tkd's per-box cost ~503 → ~392 →
  ~90 µs at 100k. Only ikd's bg-on mode is faster, and only by offloading the
  rebuild to a second core (wall < CPU); single-threaded, tkd leads.
- **ikd's second core only earns its keep on big batched rebuilds.** On the bulk
  churn bg-on shows wall < CPU but spends ~33% more CPU, and tkd still wins wall
  ...
  1500-point offload threshold, so bg-on ≈ bg-off (wall ≈ CPU) and tkd leads.
- **Net:** tkd wins every workload single-threaded — insert, kNN, radius, spatial
