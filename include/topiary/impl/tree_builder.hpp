...
public:
    ...
    /// @brief Touched-node buffer the mutators append to; drained by `maybe_partial_rebuild`.
    std::vector<std::uint32_t>& modified_nodes() { return modified_nodes_; }
    ...
    /// @brief Release every live index inside the AABB [min_corner, max_corner].
    /// @return Count of indices cleared.
    std::size_t delete_in_box(std::uint32_t root, const Point& min_corner, const Point& max_corner);
