...
class LeafBucket {
public:
    /// @brief Construct with reserved capacity (~ 2 * N: each leaf is allocated with cap = 2B).
    ...
    /// @brief Append an entry to a bucket; false if `size == capacity` (caller triggers leaf split).
